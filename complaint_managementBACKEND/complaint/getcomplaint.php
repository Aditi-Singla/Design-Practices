...
if (verify($inputarray))
{
	...
	if ($stmt->num_rows > 0)
	{
		...
		$stmt3 = $conn->prepare("SELECT vote FROM votes WHERE (complaint_id = ? AND username = ?)");
		$stmt3->bind_param("is",$complaint_id,$current);
		$stmt3->execute();
		$stmt3->store_result();
		$stmt3->bind_result($vote);
		$stmt3->fetch();
		$stmt3->free_result();
		$stmt3->close();
		if ($vote === NULL)
		...
		$tags = "";
		$stmt4 = $conn->prepare("SELECT tag FROM tags WHERE (complaint_id = ?)");
		$stmt4->bind_param("i",$complaint_id);
		$stmt4->execute();
		$stmt4->bind_result($tag);
		while($stmt4->fetch())
		{
			$tags .= "   " . $tag;
		}
